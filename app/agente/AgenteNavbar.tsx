...
export default function AgenteNavbar({ user, userRole, initialProfile }: { user: any, userRole: string, initialProfile: any }) {
  ...
  const navLinks = [
    { name: 'Dashboard', href: '/agente', exact: true },
    { name: 'Interesados', href: '/agente/leads', exact: false },
    { name: 'Visitas', href: '/agente/visits', exact: false },
          ...
          <div className="flex justify-between h-16">
            <div className="flex">
              ...
              <div className="hidden md:ml-10 md:flex md:space-x-8">
                {navLinks.map((link) => {
                  const isActive = link.exact ? pathname === link.href : pathname.startsWith(link.href) && (link.href !== '/agente' || pathname === '/agente');
                  return (
                    <Link
                      key={link.name}
                      href={link.href}
