...
async function check() {
  ...
  console.log("Casona:", casona);
  
  if (casona) {
    const { data: assignments } = await supabase.from('property_assignments').select('*, role_types(name)').eq('property_id', casona.id);
    ...
    // Check user profiles inner join
    const { data: vsInner, error } = await supabase
      .from('visits')
      .select('visit_date, status, user_profiles!inner(full_name), properties!inner(title)')
      .eq('property_id', casona.id);
    console.log("Visits inner join result:", vsInner, error);
  }
}
